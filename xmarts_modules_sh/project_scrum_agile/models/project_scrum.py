        ...
        for meeting in self:
            if meeting.duration:
                ...
                test = datetime.strftime(
                    vals.get('start_datetime'), DEFAULT_SERVER_DATETIME_FORMAT)
                stop_date = \
                    datetime. \
                    strptime(test,
...
class ScrumMeeting(models.Model):
    ...
    def create(self, vals):
        ...
        if vals.get('start_datetime') and vals.get('duration'):
            ...
            if isinstance(vals.get('start_datetime'), str):
                test = vals['start_datetime'] = vals.get('start_datetime')
            else:
                test = datetime.strftime(
                    vals.get('start_datetime'), DEFAULT_SERVER_DATETIME_FORMAT)
        if vals.get('start_datetime') and vals.get('duration'):
        ...
        if vals.get('start'):
            if isinstance(vals.get('start_datetime'), str):
                test1 = vals.get('start')
            else:
                test1 = datetime.strftime(
                    vals.get('start'), DEFAULT_SERVER_DATETIME_FORMAT)
            stop_date = \
                datetime. \
                strptime(test1,
                         DEFAULT_SERVER_DATETIME_FORMAT) + timedelta(
    ...
    def onchange_start_date(self):
        if self.allday:
            ...
            today = str(datetime.now())[:19]
            display_date_result = datetime.strftime(pytz.utc.localize(
                datetime.strptime(
                    today, DEFAULT_SERVER_DATETIME_FORMAT)).astimezone(
                local), "%Y-%m-%d %H:%M:%S")
            date_conv = datetime.strptime(
                display_date_result, DEFAULT_SERVER_DATETIME_FORMAT).time()
            self.stop = datetime.combine(
                self.stop_date, date_conv)
