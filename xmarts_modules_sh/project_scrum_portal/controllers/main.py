...
class CustomerPortal(CustomerPortal):
    ...
    def my_meetings(self, **post):
        ...
        for meeting in meetings:
            if not meeting.allday:
                if dict.get(meeting.start_datetime[:10], False):
                    dict[meeting.start_datetime[:10]].append(meeting.id)
                else:
                    dict.update({meeting.start_datetime[:10]: [meeting.id]})
            else:
    ...
    #    @http.route('/get_issue_data', type='json', auth='user', website=True)
    #    def get_issue_data(self, **post):
    #        issue_list = []
    #        stg_new = {}
    #        if post.get('project_id'):
    #            domain = [('project_id', '=', int(post.get('project_id')))]
    ...
    #            for issue in issues:
    #                if issue.stage_id in stg_new:
    #                    stg_new[issue.stage_id].append(issue.stage_id.id)
    #                else:
    #                    stg_new[issue.stage_id] = [issue.stage_id.id]
    #            if stg_new:
    #                for key, value in stg_new.items():
    #                    data = {
    #                              'label' : key.name,
